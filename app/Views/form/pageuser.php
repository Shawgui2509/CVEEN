...
<head>
    ...
    <style>
        * { font-family: 'Poppins', sans-serif; }
        .user-shell {
            background:
                radial-gradient(circle at top left, rgba(255, 56, 92, 0.12), transparent 28%),
                radial-gradient(circle at top right, rgba(17, 24, 39, 0.08), transparent 28%),
                linear-gradient(180deg, #f9fafb 0%, #ffffff 100%);
        }
    </style>
</head>
<body class="user-shell text-gray-900 min-h-screen">
<?php
$isAdmin = strtolower(trim((string) session()->get('role'))) === 'admin';
$reservationCount = isset($tabReservation) && is_array($tabReservation) ? count($tabReservation) : 0;
?>
    ...
    <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <h1 class="text-lg font-bold text-gray-900">Mes réservations</h1>
            </div>
        </div>
...
<main class="max-w-7xl mx-auto px-6 py-10 lg:py-14">
    <section class="rounded-[2rem] bg-gradient-to-r from-gray-900 via-gray-800 to-[#ff385c] text-white shadow-2xl overflow-hidden">
        <div class="p-8 lg:p-12">
            <p class="text-sm uppercase tracking-[0.35em] text-white/70 font-semibold mb-4">Espace client</p>
            <h2 class="text-3xl md:text-4xl font-bold leading-tight mb-4">Suivez vos réservations dans une interface cohérente avec l’accueil.</h2>
            <p class="text-white/80 max-w-2xl">Annulez une demande en attente ou supprimez définitivement une réservation annulée, en gardant une vue claire sur chaque statut.</p>
        </div>
    </section>
    ...
    <section class="mt-8 rounded-[2rem] bg-white shadow-xl border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h3 class="text-xl font-bold text-gray-900">Historique de réservation</h3>
                <p class="text-sm text-gray-500">Même identité visuelle que l’accueil, avec des actions claires.</p>
            </div>
            <span class="inline-flex items-center gap-2 rounded-full bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-700">
                <i class="fa-solid fa-layer-group text-[#ff385c]"></i>
                <?= esc((string) $reservationCount) ?> réservation(s)
            </span>
        </div>
                ...
                <?php if (isset($tabReservation) && !empty($tabReservation)): ?>
                    <?php foreach ($tabReservation as $LesReservations): ?>
                        ...
                        <tr class="hover:bg-gray-50/80 transition">
                            ...
                            <td class="px-6 py-5">
                                <?php if ($state === 'En attente de validation' || $state === 'Modifiée' || $state === 'Modifiee'): ?>
                                    <form method="post" action="<?= site_url('PageUser') ?>" class="flex flex-wrap gap-3">
                                ...
                                <?php elseif ($state === 'Annulee' || $state === 'Annulée'): ?>
                                    <form method="post" action="<?= site_url('PageUser') ?>" onsubmit="return confirm('Supprimer definitivement cette reservation ?');">
                                        <input type="hidden" name="idReservation" value="<?= esc((string) ($LesReservations['id_reservation'] ?? '')) ?>">
                                        <button type="submit" name="action" value="supprimer" class="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-white px-4 py-2 text-sm font-semibold text-rose-600 hover:bg-rose-50 transition" title="Supprimer">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-16 text-center">
                            <div class="mx-auto max-w-md">
                                ...
                                <p class="text-sm text-gray-500">Vos réservations apparaîtront ici avec les actions de gestion associées.</p>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
